...
void Adult::UpdateProbTransmM()
{
	...
	if (MSMind == 0 && Risk==1){ 
		...
		//	(1.0 + 4.0 * ClientPrev * (1.0 - ClientARTcov) * DurFSW[CurrYear - 1985]));
		BaseProb[8] = TransmFSW[1] * RelInfecRiskST[0][0][1] * (1.0 + (RRclientToFSW1985 - 1.0) * FSWselect); // NEW: open cohort frailty FSW
...
void SetInitialParameters()
{
	...
	FSWselect = 1.0; // NEW: no selection at epidemic start
...
void UpdateFSWdemand()
{
	...
	if(CurrMonth==0){
		...
		//  NEW: moment-ODE update of the FSW selection factor
		//double dt_sel = 1.0/12.0; // monthly step
		double nu_sel = 1.0 / DurFSW[CurrYear - 1985]; //FSW turnover approx 1/mean duration
		double frc_sel = ClientPrev * (1.0 - ClientARTcov); // infectious-client force
		for (int msel = 0; msel < 12; msel++){
			FSWselect += (1.0 / 12.0) * (nu_sel * (1.0 - FSWselect) - 15.0 * frc_sel * FSWselect * FSWselect);
			if (FSWselect < 0.0){ FSWselect = 0.0; }
			if (FSWselect > 1.0){ FSWselect = 1.0; }
		}
		if (FixedUncertainty == 1){
			PrevClients.out[CurrSim - 1][CurrYear - 1985] = ClientPrev;
			FSWselectOut.out[CurrSim - 1][CurrYear - 1985] = FSWselect; //NEW]
...
void UpdateFSW()
{
	...
	MnthlyExit = 1.0 - exp(-1/(12.0*DurFSW[iy]));
	for(ia=0; ia<81; ia++){
...
*/
// HIV prevalence in sex workers || Nina Anderegg new (IRR) -> old version above
if(FixedUncertainty==1 || CalibFSWprev==1){
  Temp1 = 0.0;
  Temp2 = 0.0;
  for(ia=0; ia<81; ia++){
    NegFSW_byAge_byYear[ia][iy] = 0.0; 
    Temp1 += FH_SW.Total[ia];
    double neg_fsw_age =
      FH_SW.NegNoHCT[ia] + FH_SW.NegPastHCT[ia] + FH_SW.RegHCT[ia] +
      FH_SW.RegPrEP[ia] + FH_SW.RegVM[ia];
    
    NegFSW_byAge_byYear[ia][iy] = neg_fsw_age; 
    Temp2 += neg_fsw_age;
    if (ia == 15){
      Temp3 = Temp1;
      Temp4 = Temp2;
...
void ResultsAtEndOfYr2()
{
	...
	// IRR (age-maged) FSW to general females // Nina Anderegg new (IRR)
	
	double denom_w = 0.0, R_gen_std = 0.0; 
	for (ia = 0; ia < 81; ++ia){
	  denom_w += NegFSW_byAge_byYear[ia][iy];}
	if (denom_w > 0.0){
	  for (ia = 0; ia < 81; ++ia){
	    double w = NegFSW_byAge_byYear[ia][iy] / denom_w;
	    double inc_f_age = NewHIVbyAgeSex[ia][1] / (TotalPop_S[ia+10][1] - TotalPositive_S[ia+10][1]);
	    R_gen_std += w * inc_f_age;
	  }
	}
	GenIncStd_FSWweights.out[CurrSim-1][iy] = R_gen_std;
	IRR_FSWtoF_agematched.out[CurrSim-1][iy] = (R_gen_std > 0.0 ? HIVincFSW.out[CurrSim-1][iy] / R_gen_std : 0.0);
	
	double New_HIV_tot_f = 0.0, Total_Susc_f = 0;
	for (ia = 0; ia < 81; ++ia){
	  New_HIV_tot_f += NewHIVbyAgeSex[ia][1];
	  Total_Susc_f += (TotalPop_S[ia+10][1] - TotalPositive_S[ia+10][1]);
	}
	GenInc_nonstd.out[CurrSim-1][iy] = New_HIV_tot_f/Total_Susc_f;
	IRR_FSWtoF_nonmatched.out[CurrSim-1][iy] = (New_HIV_tot_f/Total_Susc_f > 0.0 ? HIVincFSW.out[CurrSim-1][iy] / (New_HIV_tot_f/Total_Susc_f) : 0.0);
	
	double New_HIV_tot_f_1539 = 0.0, Total_Susc_f_1539 = 0;
	for (ia = 5; ia < 30; ++ia){
	  New_HIV_tot_f_1539 += NewHIVbyAgeSex[ia][1];
	  Total_Susc_f_1539 += (TotalPop_S[ia+10][1] - TotalPositive_S[ia+10][1]);
	}
	
	GenInc_nonstd_1539.out[CurrSim-1][iy] = New_HIV_tot_f_1539/Total_Susc_f_1539;
	IRR_FSWtoF_nonmatched_1539.out[CurrSim-1][iy] = (New_HIV_tot_f_1539/Total_Susc_f_1539 > 0.0 ? HIVincFSW.out[CurrSim-1][iy] / (New_HIV_tot_f_1539/Total_Susc_f_1539) : 0.0);
...
void RunSample()
{
	...
	PrevClients.RecordSample("PrevClients.txt"); //NEW
	ARTcoverageClients.RecordSample("ARTcoverageClients.txt"); //NEW
	PAFforCSW.RecordSample("PAFforCSW.txt");
	NewHIVclients.RecordSample("NewHIVclients.txt"); // Nina Anderegg new (PAF)
	NewHIVinFSW.RecordSample("NewHIVinFSW.txt"); // Nina Anderegg new (PAF)
	NewAdultHIV.RecordSample("NewAdultHIV.txt"); // Nina Anderegg new (PAF)
	GenIncStd_FSWweights.RecordSample("GenIncStd_FSWweights.txt"); // Nina Anderegg new (IRR)
	IRR_FSWtoF_agematched.RecordSample("IRR_FSWtoF_agematched.txt"); // NIna Anderegg new (IRR)
	GenInc_nonstd.RecordSample("GenInc_nonstd.txt"); // Nina Anderegg new (IRR)
	IRR_FSWtoF_nonmatched.RecordSample("IRR_FSWtoF_nonmatched.txt"); // NIna Anderegg new (IRR)
	FSWselectOut.RecordSample("FSWselect.txt"); // NEW
