...
void Adult::UpdateProbTransmM()
{
	...
	if (MSMind == 0 && Risk==1){ 
		...
		BaseProb[8] = TransmFSW[1] * RelInfecRiskST[0][0][1] * (1.0 + (RRclientToFSW1985 - 1.0) * FSWselect);
...
void UpdateFSWdemand()
{
	...
	if(CurrMonth==0){
		...
		//double dt_sel = 1.0/12.0;
		double nu_sel = 1.0 / DurFSW;
		double frc_sel = ClientPrev * (1.0 - ClientARTcov);
